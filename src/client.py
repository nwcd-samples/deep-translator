...
class Client:
    """
    Represents a templates connected to the VoiceStreamAI server.

    This class maintains the state for each connected templates, including their
    unique identifier, audio buffer, configuration, and a counter for processed
    audio files.

    Attributes:
        client_id (str): A unique identifier for the templates.
        buffer (bytearray): A buffer to store incoming audio data.
        config (dict): Configuration settings for the templates, like chunk length
                       and offset.
